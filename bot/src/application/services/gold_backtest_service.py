...
class GoldBacktestService:
    ...
    def _log_backtest_exit_summary(self, result: dict[str, Any], source_name: str, artifact_stem: str) -> None:
        ...
        logger.info(
            "🧪 Backtest execution model | fill_model=%s slippage_pips=%.2f spread_pips=%.2f",
            self.settings.backtest_fill_model,
            self.settings.backtest_slippage_pips,
            self.settings.backtest_spread_pips,
        )
    ...
    def _estimate_trade_outcome(
        ...
        slippage_pips = max(0.0, float(getattr(self.settings, "backtest_slippage_pips", 1.0)))
        spread_pips = max(0.0, float(getattr(self.settings, "backtest_spread_pips", 0.5)))
        fill_model = str(getattr(self.settings, "backtest_fill_model", "next_bar_close")).strip().lower()

        next_bar = lower_frame.iloc[idx + 1]
        next_open = float(next_bar.get("open", next_bar.get("close", entry_price)))
        next_close = float(next_bar.get("close", entry_price))
        next_high = float(next_bar.get("high", next_close))
        next_low = float(next_bar.get("low", next_close))

        if fill_model == "next_bar_open":
            fill_price = next_open
        elif fill_model == "mid_price":
            fill_price = (next_high + next_low) / 2.0
        else:
            fill_price = next_close

        if direction.lower() == "buy":
            fill_price += (spread_pips * pip_size) + (slippage_pips * pip_size)
            exit_price = next_close
            if next_low <= entry_price - (sl_pips * pip_size):
                exit_price = max(entry_price - (sl_pips * pip_size), next_low)
            elif next_high >= entry_price + (tp_pips * pip_size):
                exit_price = min(entry_price + (tp_pips * pip_size), next_high)
        else:
            fill_price -= (spread_pips * pip_size) + (slippage_pips * pip_size)
            exit_price = next_close
            if next_high >= entry_price + (sl_pips * pip_size):
                exit_price = min(entry_price + (sl_pips * pip_size), next_high)
            elif next_low <= entry_price - (tp_pips * pip_size):
                exit_price = max(entry_price - (tp_pips * pip_size), next_low)

        move = exit_price - fill_price if direction.lower() == "buy" else fill_price - exit_price
        pips = move / pip_size
